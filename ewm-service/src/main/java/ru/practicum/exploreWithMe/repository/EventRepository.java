...
public interface EventRepository extends JpaRepository<Event, Long> {
    @Query(value = "SELECT e FROM Event e WHERE (e.initiator IN ?1 OR ?1 IS NULL) AND (e.state IN ?2 or ?2 IS NULL) AND (e.category IN ?3 or ?3 IS NULL) AND e.eventDate BETWEEN ?4 AND ?5")
